...
#' @param panel.names the names for the panels in the same
#' order as the stargazer output
#' @param reg (logical) set to TRUE if the panels are regression output;
#' defualt is TRUE
#' @param same.lhs.vars (logical) set to TRUE if all panels have the
#' same left-hand-side variables. Currently, only TRUE is supported
#' @param same.summary.stats (logical) set to TRUE if all panels have the
#' same summary stats. Currently, only TRUE is supported
...
#' }
#' @export
star_panel <- function(..., starlist = NULL, panel.names, reg = TRUE, same.lhs.vars = TRUE,
